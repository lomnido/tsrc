...
def test_raw_dump_from_abs_path(
    ...
    * 13th: if we move "repo 3", than COMMON PATH calculation will
    ...
    test_path_1: str = os.path.join(
        str(workspace_path), "common path lvl1", "manifest.yml"
    )
    assert message_recorder.find(rf"=> Creating NEW file '{test_path_1}'")
    ...
    test_path_2: str = os.path.join("common path lvl1", "manifest.yml")
    print("DEBUG test_path_2 =", test_path_2)
    assert message_recorder.find(
        rf"Error: Such file '{test_path_2}' already exists, use '--force' if you want to overwrite it"
    )
    ...
    tsrc_cli.run("dump-manifest", "--raw", ".")
    test_path_3: str = os.path.join("common path lvl1", "level 2", "manifest.yml")
    assert message_recorder.find(rf"=> Creating NEW file '{test_path_3}'")
...
def test_raw_dump__workspace__combined_path(
    ...
    * 4th: test if '--workspace' gets combined with RAW dump path
    ...
    # 4th: test if '--workspace' gets combined with RAW dump path
    assert message_recorder.find(
        rf":: Checking Path \(recursively\) for Repos from: {sub1_path}"
    )
    assert message_recorder.find(rf"=> Using Repo\(s\) COMMON PATH on: '{sub1_path}'")
    test_path_1: str = os.path.join(sub1_path, "repo 1")
    assert message_recorder.find(
        rf"Warning: No remote found for: 'repo 1' \(path: '{test_path_1}'\)"
    )
